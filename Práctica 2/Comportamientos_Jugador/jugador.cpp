...
Action ComportamientoJugador::think(Sensores sensores) {
	...
	//Actualiza el efecto de la última acción
	switch (ultimaAccion)
	{
	...
	//Compruebo si ha cambiado el destino
	...
	if(hayPlan and plan.size() > 0){//Hay un plan y hay que seguirlo
		...
		if(sensores.terreno[2] == 'P' or sensores.terreno[2] == 'M' or 
			sensores.terreno[2] == 'D' or sensores.superficie[2]=='a'){ //No hay plan y se activa un comportamiento reactivo
		sigAccion = actTURN_R;
	}
	else{
...
bool ComportamientoJugador::pathFinding_CostoUniforme(const estado &origen, const estado &destino, list<Action> &plan){
	...
	cout << "Calculando plan\n";
	plan.clear();
	set<estado,ComparaEstados> explorados; // Lista de nodos explorados
	...
	while (!cola.empty() and (current.st.fila!=destino.fila or current.st.columna != destino.columna)){
		...
		// Generar descendiente de girar a la derecha
		nodo hijoTurnR = current;
		set<estado,ComparaEstados>::iterator itR;
		hijoTurnR.st.orientacion = (hijoTurnR.st.orientacion+1)%4;
		itR = explorados.find(hijoTurnR.st);
		if(itR == explorados.end()){
			hijoTurnR.secuencia.push_back(actTURN_R);
			hijoTurnR.coste += 1; //Coste del giro
			cola.push(hijoTurnR);
		}
		...
		// Generar descendiente de girar a la izquierda
		nodo hijoTurnL = current;
		set<estado,ComparaEstados>::iterator itL;
		hijoTurnL.st.orientacion = (hijoTurnL.st.orientacion+3)%4;
		itL = explorados.find(hijoTurnL.st);
		if(itL == explorados.end()){
			hijoTurnL.secuencia.push_back(actTURN_L);
			hijoTurnL.coste += 1;//Coste del giro
			cola.push(hijoTurnL);
		}
		...
		// Generar descendiente de avanzar
		nodo hijoForward = current;
		set<estado,ComparaEstados>::iterator itF = explorados.find(hijoForward.st);
		if (!HayObstaculoDelante(hijoForward.st)){
			if ( explorados.find(hijoForward.st) == explorados.end()){
				hijoForward.secuencia.push_back(actFORWARD);
				hijoForward.coste += getCosto(mapaResultado[hijoForward.st.fila][hijoForward.st.columna]);
				cola.push(hijoForward);
			}	
		}	
